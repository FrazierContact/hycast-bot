...
client.on("message", async function (message) {
    ...
    if (command === "ticket" && botConfigs.plugins[12].activated == false) {
        ...
        if (message.channel.id === botConfigs.ticketsystem.createTicketChannelID) {
            ...
            message.author.send("-=HyCast Support=- \nA Ticket has been created! We appreciate you contacting our support team. A staff member will get back to you shortly.\nHappy Hytaling,\nHyCast");
            message.delete().catch(O_o => { });
        } else {
            ...
            if (message.channel.parent.id === botConfigs.ticketsystem.ticketCategoryID) {
                ...
                if (message.channel.name === message.author.id) {
                    ...
                    client.fetchUser(message.channel.name)
                        .then(user => {
                            user.send("-=HyCast Support=-\nYour ticket has been closed by an a Staff Member.\n~Happy Hytaling")
                        })
                }
            } else {
